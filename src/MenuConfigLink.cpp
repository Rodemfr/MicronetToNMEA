...
void MenuConfigLink()
{
    ...
    gnssConfigIndex    = FindLinkIndex(gnssConfigTable, sizeof(gnssConfigTable), gConfiguration.gnssSource);
    windConfigIndex    = FindLinkIndex(windConfigTable, sizeof(windConfigTable), gConfiguration.windSource);
    depthConfigIndex   = FindLinkIndex(depthConfigTable, sizeof(depthConfigTable), gConfiguration.depthSource);
    speedConfigIndex   = FindLinkIndex(speedConfigTable, sizeof(speedConfigTable), gConfiguration.speedSource);
    compassConfigIndex = FindLinkIndex(compassConfigTable, sizeof(compassConfigTable), gConfiguration.compassSource);
    ...
    while (!exitLoop)
    {
        ...
        if ((c >= 0x30) && (c <= 0x39))
        {
            ...
            case 4:
                ...
                break;
            case 5:
                gConfiguration.gnssSource = (LinkId_t)gnssConfigTable[gnssConfigIndex];
                gConfiguration.windSource = (LinkId_t)windConfigTable[windConfigIndex];
                gConfiguration.depthSource = (LinkId_t)depthConfigTable[depthConfigIndex];
                gConfiguration.speedSource = (LinkId_t)speedConfigTable[speedConfigIndex];
                gConfiguration.compassSource = (LinkId_t)compassConfigTable[compassConfigIndex];
