...
/**
 * @class MicronetCodec
 * @brief Parses and encodes Micronet protocol messages and exposes navigation data.
 *
 * MicronetCodec is responsible for decoding raw Micronet messages received from
 * devices on a Micronet network and translating them into higher-level
 * NavigationData that can be emitted on an NMEA network. It interprets message
 * headers and payloads, extracts device and network identifiers, and decodes
 * the various Micronet data fields (time, position, speed, wind, depth, etc.).
 *
 * In addition to parsing incoming traffic, the class can construct and encode
 * outbound Micronet messages (data messages, slot requests/updates, acknowledgements,
 * pings, resets) and build a NetworkMap describing transmission slots and timing.
 * Utility functions include signal-strength calculation, packing/unpacking of
 * Micronet-specific field formats, and computation of derived navigation values
 * such as true wind.
 *
 * The class maintains an internal NavigationData instance representing the
...
class MicronetCodec
{
  public:
    ...
    uint8_t EncodeMasterRequest(MicronetMessage_t *message, uint8_t signalStrength, uint32_t networkId, uint32_t deviceId, uint32_t nbSlaves, const uint32_t *slaveIds, const uint8_t *payloadLens);
