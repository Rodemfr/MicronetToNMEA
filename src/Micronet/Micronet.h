...
#define MICRONET_DEVICE_TYPE_DUAL_MAXI_DISPLAY   0x86
#define MICRONET_DEVICE_TYPE_COMPASS             0x87
#define MICRONET_DEVICE_TYPE_REMOTE_DISPLAY      0x88
...
/**
 * @struct MicronetMessage_t
 * @brief Container for Micronet RF messages, transmission scheduling and control.
 *
 * This structure is used both to store messages received from the RF interface
 * and to prepare messages for transmission. It is also used to express simple
 * RF control actions such as enabling/disabling the receiver or changing power
 * state. The fields describe the requested action, payload length, signal
 * strength, the time window during which the RF operation applies, and the raw
 * message bytes.
 */
#define MICRONET_ACTION_RF_NO_ACTION    0
#define MICRONET_ACTION_RF_TRANSMIT     1
#define MICRONET_ACTION_RF_LOW_POWER    2
#define MICRONET_ACTION_RF_ACTIVE_POWER 3

typedef struct
{
    uint8_t  action;                            // Action or operation to perform on RF subsystem.
    uint8_t  len;                               // Number of valid bytes in the data[] array (payload length)
    int16_t  rssi;                              // Received Signal Strength Indicator (RSSI) for received messages
    uint32_t startTime_us;                      // Start time of the RF action/window in microseconds
    uint32_t endTime_us;                        // End time of the RF action/window in microseconds
    uint8_t  data[MICRONET_MAX_MESSAGE_LENGTH]; // Raw message buffer used to hold incoming or outgoing Micronet message bytes. Only the first 'len'
                                                // bytes are valid
} MicronetMessage_t;
