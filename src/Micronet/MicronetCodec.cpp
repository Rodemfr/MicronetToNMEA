...
/**
 * @brief Construct a new MicronetCodec instance.
 *
...
/**
 * @brief Destroy the MicronetCodec instance.
 *
 * Clean up resources if required (currently none).
 */
MicronetCodec::~MicronetCodec()
{
}
...
/**
 * @brief Extract network identifier from a Micronet message header.
 * @param message Pointer to Micronet message
 * @return 32-bit network identifier
 */
...
/**
 * @brief Return device type field from message header.
 * @param message Pointer to Micronet message
 * @return Device type byte
 */
...
/**
 * @brief Extract device identifier from a Micronet message header.
 * @param message Pointer to Micronet message
 * @return 32-bit device identifier
 */
...
/**
 * @brief Return message-id field from message header.
 * @param message Pointer to Micronet message
 * @return Message id byte
 */
...
/**
 * @brief Return data-field/source identifier from message header.
 * @param message Pointer to Micronet message
 * @return Data/source id byte
 */
...
/**
 * @brief Return stored signal strength byte from message header.
 * @param message Pointer to Micronet message
 * @return Signal strength byte
 */
...
/**
 * @brief Return header CRC byte from message header.
 * @param message Pointer to Micronet message
...
/**
 * @brief Verify header CRC and basic length consistency of a message.
 * @param message Pointer to Micronet message
 * @return true if header CRC and length fields are valid, false otherwise
 */
...
/**
 * @brief Decode a complete Micronet message and update navigation state.
 * @param message Pointer to Micronet message
 * @return true if an acknowledgement should be sent for this message, false otherwise
 *
 * Dispatches handling based on message type and returns whether an ack is requested.
 */
...
/**
 * @brief Decode the payload of a SEND_DATA message.
 * @param message Pointer to Micronet message
 *
 * Iterates through payload fields and decodes each field into navData.
 */
...
/**
 * @brief Decode the payload of a SET_PARAMETER message.
 * @param message Pointer to Micronet message
 *
 * Validates payload CRC and dispatches to specific parameter page handlers.
 */
...
/**
 * @brief Handle configuration page 0xFF in parameter messages.
 * @param message Pointer to Micronet message
 *
 * Interprets various calibration identifiers and updates navData calibration fields.
 */
...
/**
 * @brief Decode a single data field starting at the given offset.
 * @param message Pointer to Micronet message
 * @param offset Byte offset within message where field starts
 * @return New offset to continue parsing (or negative on error)
 *
 * Handles different Micronet field types, verifies field CRC and updates navData.
 */
...
/**
 * @brief Update navigation data with an 8-bit field value.
 * @param fieldId Field identifier
 * @param value Signed 8-bit value
 *
 * Interprets small-size Micronet fields and updates corresponding navData entries.
 */
...
/**
 * @brief Update navigation data with a 16-bit field value.
 * @param fieldId Field identifier
 * @param value Signed 16-bit value
 *
 * Handles interpretation (units/scaling) for various 16-bit Micronet fields.
 */
...
/**
 * @brief Update navigation data with two 32-bit values (used for longer fields).
 * @param fieldId Field identifier
 * @param value1 First 32-bit value
 * @param value2 Second 32-bit value
 *
 * Typically used for trip/log fields, splitting returned values across two outputs.
 */
...
/**
 * @brief Get total message length for given data fields
...
/**
 * @brief Encode a data message with navigation data
                                        ...
                                        uint32_t dataFields)
{
    ...
    {
        offset +=
            Add24bitField(message->data + offset, MICRONET_FIELD_ID_DATE, (navData.date.day << 16) + (navData.date.month << 8) + navData.date.year);
    }
    ...
    if ((dataFields & DATA_FIELD_NODE_INFO))
    {
        offset += AddQuad8bitField(message->data + offset, MICRONET_FIELD_ID_NODE_INFO, MNET2NMEA_SW_MINOR_VERSION, MNET2NMEA_SW_MAJOR_VERSION, 0x33,
                                   signalStrength);
    }
...
/**
 * @brief Encode a slot update message
 * @param message Pointer to message buffer to encode into
 * @param signalStrength Signal strength value (0-9)
 * @param networkId Network identifier
 * @param deviceId Device identifier
...
/**
 * @brief Encode a slot request message
 * @param message Pointer to message buffer to encode into
 * @param signalStrength Signal strength value (0-9)
 * @param networkId Network identifier
 * @param deviceId Device identifier
...
/**
 * @brief Encode an acknowledge parameter message
 * @param message Pointer to message buffer to encode into
 * @param signalStrength Signal strength value (0-9)
 * @param networkId Network identifier
 * @param deviceId Device identifier
...
/**
 * @brief Write message length and calculate header CRC
 * @param message Pointer to message to finalize
 *
 * Updates length fields and calculates/writes header CRC for a message
 */
...
/**
 * @brief Add a 16-bit field to a message
 * @param buffer Pointer to message buffer position to write to
 * @param fieldCode Field identifier code
...
/**
 * @brief Add a 24-bit field to a message
 * @param buffer Pointer to message buffer position to write to
 * @param fieldCode Field identifier code
 * @param value 24-bit value to encode
 * @return Number of bytes written
 */
...
/**
 * @brief Add two 16-bit fields to a message
 * @param buffer Pointer to message buffer position to write to
 * @param fieldCode Field identifier code
 * @param value1 First 16-bit value to encode
 * @param value2 Second 16-bit value to encode
 * @return Number of bytes written
 */
...
/**
 * @brief Add four 8-bit fields to a message
 * @param buffer Pointer to message buffer position to write to
 * @param fieldCode Field identifier code
 * @param value1 First 8-bit value
 * @param value2 Second 8-bit value
 * @param value3 Third 8-bit value
 * @param value4 Fourth 8-bit value
 * @return Number of bytes written
 */
...
/**
 * @brief Add one 16-bit and six 8-bit fields to a message (used for waypoint data)
 * @param buffer Pointer to message buffer position to write to
 * @param fieldCode Field identifier code
 * @param value1 16-bit value (typically bearing)
 * @param wpName Pointer to waypoint name characters
 * @param wpNameLength Length of waypoint name
 * @return Number of bytes written
 */
...
/**
 * @brief Add a 32-bit field to a message
 * @param buffer Pointer to message buffer position to write to
 * @param fieldCode Field identifier code
...
/**
 * @brief Add position data (latitude/longitude) to a message
 * @param buffer Pointer to message buffer position to write to
...
/**
 * @brief Extract network mapping from a master request message
 * @param message Pointer to received master request message
 * @param networkMap Pointer to structure to store network mapping
 * @return true if mapping successfully extracted, false otherwise
 */
...
/**
 * @brief Get start time of current network cycle
 * @param networkMap Current network mapping
...
/**
 * @brief Get start time of next network cycle
 * @param networkMap Current network mapping
 * @return Start time in microseconds of next cycle
 */
...
/**
 * @brief Get end time of current network cycle
 * @param networkMap Current network mapping
...
/**
 * @brief Calculate signal strength value from RSSI
 * @param message Message containing RSSI measurement
 * @return Signal strength value from 0 (weakest) to 9 (strongest)
 *
 * Converts RSSI values to Micronet signal strength scale:
 * - RSSI < -95: 0
 * - RSSI < -90: 1
 * - RSSI < -85: 2
 * And so on up to 9 for RSSI >= -55
 */
