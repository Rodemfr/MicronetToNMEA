...
To work properly, MicronetToNMEA needs at least a ESP32 WROOM DevKitC board and a CC1101 based breakout board.

\subsection{ESP32 Board}

There are many ESP32 microcontroller variants as well as many ESP32 based boards. Not all of them are suitable for MicronetToNMEA. Bluetooth Classic SPP is required and only the D0WD version of ESP32 supports it. \textbf{Newer ESP32-S3/C2/C3/H2 don't support Bluetooh Classic and will not work with MicronetToNMEA}.

The simplest choice is to use a WROOM DevKitC board like this one: \href{https://www.upesy.fr/products/upesy-esp32-wroom-devkit-board}{uPesu Wroom DevKit}.
ESP32 boards are very easy to find and purchase and are very cheap.
...
If you already have an AIS with a NMEA0183 output on your boat you can use it as a GNSS source. MicronetToNMEA also have the capability to forward AIS data to the PC/Tablet. Be careful that AIS outputs are not 3.3V and that you will likely need a level shifter or a RS422/485 transceiver to protect your ESP32.

\subsection{LSM303DLH, LSM303DLHC or LSM303AGR navigation compass breakout board}

Connected to Teensy I2C bus, this IC will allow getting magnetic heading. MicronetToNMEA automatically detects the presence and type of LSM303DLH/DLHC/AGR on its I2C bus.

\section{Required software}

\subsection{Visual Studio Code and PlatformIO}

MicronetToNMEA has been developped with \href{https://code.visualstudio.com/}{Visual Studio Code} with PlatformIO extension. All VS Code \& PlatformIO configuration files are stored into the repository making project import very easy. MicronerToNMEA can not be compiled with ArduinoIDE.
...
\item At the first opening, PlatformIO plugin will download and install ESP32 toolchain. This can take a while.
\item Once PlatformIO is ready, you can compile MicronetToNMEA by pressing \emph{SHIFT-CTRL-B}.
\item To upload the compiled binary to the ESP32, press \emph{SHIFT-CTRL-U}.
\end{itemize}

\section{Compile time configuration}
\label{compile-time-configuration}

By default, MicronetToNMEA is configured for a specific HW layout. This means that it is configured to be connected through specific SPI, I2C or GPIO pins to the default ESP32 board. This configuration can be changed to some extent to adapt your own needs. The file bearing this configuration is “BoardConfig.h”. Note that no coherency check is made in the software, it is your responsibility to provide a reachable configuration (i.e. not to connect SPI wires to non SPI capable pins). Table {\ref{table:configswitches}} lists all available switches and their meaning.
	...
	\begin{tabularx}{\linewidth}{@{}lX@{}}
		...
		\hline
		COMPASS\_I2C & Sets the I2C bus to which the navigation compass (i.e. LSM303DLH(C)) is connected. Defined as per “Wiring” library definition (Wire0, Wire1, etc.)\\
		\hline
		BLUETOOTH\_DEVICE\_NAME & Bluetooth device name as it will be seen by other devices\\
		\hline
		...
		\hline
		CONSOLE\_BAUDRATE & Defines console baud-rate\\
		\hline
		NMEA0183\_IN\_IS\_UBLOXM8N & Enable automatic configuration of UBLOX M8N/M6N GPS when connected to NMEA input (0=disabled, 1=enabled)\\
		\hline
		NMEA0183\_IN & Defines on which serial port is connected the NMEA input  (Serial, Serial1, Serial2, etc.)\\
		\hline
		NMEA0183\_IN\_BAUDRATE & Defines NMEA UART baud-rate\\
		\hline
		PLOTTER & Defines on which Serial port is connected the plotter/navigation computer (gBTSerial or Serial)\\
		\hline
\end{tabularx}
\begin{table}[h]
...
ESP32 board must be connected to other boards with the same scheme that you have defined in “BoardConfig.h”. No check is made by MicronetToNMEA software to verify that your configuration is matching your actual connections. You must carefully verify that you properly connected the various devices since wrong connections can possibly damage your hardware, especially with respect to power supply connections.
...
This is the most straightforward way to power the system: just plug an USB cable in the ESP32 connector and it will be powered by the connected PC. ESP32 board is equipped with a voltage regulator which provides 3.3V. This 3.3V voltage can be used to power other boards of the system.
